...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& new_interval) {
        if(intervals.size() && new_interval[1] < intervals[0][0]){
        ...
        while(i<n){
            ...
            if(interval[1]>=new_interval[0]) break;
            i++;
        }
        if(i==n){
        ...
        vvi ans(intervals.begin(), intervals.begin()+i);
        vi cur_interval = intervals[i];
        
        amax(cur_interval[1], new_interval[1]);
        amin(cur_interval[0], new_interval[0]);
        while(i<n){
            if(cur_interval[1]>=intervals[i][0]){
                amax(cur_interval[1], intervals[i][1]);
            }
            else{
